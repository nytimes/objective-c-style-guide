...
Never compare something directly to `YES`, because `YES` is defined to 1, and a `BOOL` in Objective-C is a `CHAR` type that is 8 bits long (so a value of 11111110 will return NO if compared to `YES`)
